...
function RedisClient(options, stream) {
  ...
  if (stream) {
    ...
    cnx_options.family =
      (!options.family && net.isIP(cnx_options.host)) ||
  ...
  for (var command in options.rename_commands) {
    options.rename_commands[command.toLowerCase()] =
      options.rename_commands[command];
  }
  ...
  this.enable_offline_queue =
    options.enable_offline_queue === false ? false : true;
...
RedisClient.prototype.flush_and_error = function (error_attributes, options) {
  ...
  for (var i = 0; i < queue_names.length; i++) {
    ...
    if (queue_names[i] === "command_queue") {
      ...
      error_attributes.message = error_attributes.message.replace(
    ...
    for (
      var command_obj = this[queue_names[i]].shift();
      command_obj;
  ...
  if (exports.debug_mode && aggregated_errors.length) {
    ...
    if (aggregated_errors.length === 1) {
      ...
      error_attributes.message = error_attributes.message
        .replace("It", "They")
        .replace(/command/i, "$&s");
...
RedisClient.prototype.send_offline_queue = function () {
  for (
    var command_obj = this.offline_queue.shift();
    command_obj;
...
RedisClient.prototype.return_error = function (err) {
  ...
  if (command_obj.error) {
    err.stack = command_obj.error.stack.replace(
      /^Error.*?\n/,
...
function subscribe_unsubscribe(self, reply, type) {
  ...
  var buffer =
    self.options.return_buffers ||
    (self.options.detect_buffers && command_obj.buffer_args);
  ...
  if (
    command_obj.args.length === 1 ||
    self.sub_commands_left === 1 ||
    (command_obj.args.length === 0 && (count === 0 || channel === null))
  ) {
    if (count === 0) {
    ...
    if (self.sub_commands_left !== 0) {
      ...
      self.sub_commands_left = command_obj.args.length
        ? command_obj.args.length - 1
        : count;
    }
  }
...
RedisClient.prototype.internal_send_command = function (command_obj) {
  ...
  if (big_data === false) {
    ...
    debug(
      "Send " + this.address + " id " + this.connection_id + ": " + command_str
    );
...
RedisClient.prototype.write_strings = function () {
  var str = "";
  for (
    var command = this.pipeline_queue.shift();
    command;
    command = this.pipeline_queue.shift()
  ) {
